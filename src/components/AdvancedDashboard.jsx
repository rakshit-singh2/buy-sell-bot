...
const createData = (count, bnbBalance, expectedRequirement, buyHash, tokenBalance, approveHash, sellHash, expectedBnb, minBnb, status, error
) => {
  return { count, bnbBalance, expectedRequirement, buyHash, tokenBalance, approveHash, sellHash, expectedBnb, minBnb, status, error };
};
...
const Dashboard = ({ config, setConfig }) => {
  ...
  useEffect(() => {
    console.log(config.buyTransactions, config.sellTransactions, Math.max(config.buyTransactions, config.sellTransactions))
    setRows(
  ...
  const handleStartBot = async () => {
    ...
    try {
      ...
      for (i = 0; i < cycles; i++) {
        try {
          // --- BUY ---
          if (buyCount > 0 && config.buyAmounts?.[i]) {
            const buyAmount = config.buyAmounts[i];
            const buyGap = config.buyGaps?.[i] || 0;
            ...
            await new Promise(resolve => setTimeout(resolve, buyGap * 1000));
          }
  
          // --- SELL ---
          if (sellCount > 0 && config.sellAmounts?.[i]) {
            const sellAmount = config.sellAmounts[i];
            const sellGap = config.sellGaps?.[i] || 0;
            ...
            await new Promise(resolve => setTimeout(resolve, sellGap * 1000));
          }
